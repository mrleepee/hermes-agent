...
class APIServerAdapter(BasePlatformAdapter):
    ...
    @staticmethod
    def _check_signal_dispatch_auth(request: "web.Request") -> Optional["web.Response"]:
        token = os.environ.get("SIGNAL_GATEWAY_DISPATCH_TOKEN", "").strip()
        if not token:
            return web.json_response({"error": "Signal dispatch token is not configured"}, status=503)
        auth_header = request.headers.get("Authorization", "")
        if auth_header.startswith("Bearer "):
            candidate = auth_header[7:].strip()
            if hmac.compare_digest(candidate, token):
    ...
    async def _handle_internal_signal_dispatch(self, request: "web.Request") -> "web.Response":
        """POST /internal/signal/dispatch — accept Signal messages from the shared gateway.

        The shared liberland-signal-gateway (Rust) receives Signal DMs and forwards
        them here. The citizen agent processes the message through its AI pipeline
        and returns the text response synchronously in the HTTP body.
        """
        auth_err = self._check_signal_dispatch_auth(request)
        if auth_err:
            return auth_err
        ...
        try:
            ...
            return web.json_response({"ok": False, "error": "Request body must be valid JSON"}, status=400)

        # Validate required fields
        text = str(body.get("text", "")).strip()
        if not text:
            return web.json_response(
                {"ok": False, "error": "Field 'text' is required"},
                status=400,
            )
        ...
        try:
            ...
            logger.exception("Signal dispatch: agent pipeline error")
            return web.json_response({"ok": False, "error": str(e)}, status=500)

        # Extract text reply from the pipeline result
        response_text = ""
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app.router.add_post("/internal/signal/dispatch", self._handle_internal_signal_dispatch)
